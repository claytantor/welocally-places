...
function wl_self_deprecating_sidebar_registration(){
	...
	'after_title'   => '</h3>' ); 
  	...
  	$new_category_widget = array(1=> 'categories-2');
  	$new_sidebar = array('wl-sidebar-1'=> $new_category_widget);
...
function welocally_getkey() {
	...
	//set POST variables 
	$url = wl_server_base().'/rcadmin/signup/plugin/key';
	
	$result_json = wl_do_curl(
		$url,
		$selectedPostJson,
...
function wl_do_curl_http($url,$selectedPostJson, $headers){
	//open connection
	$ch = curl_init();
...
function wl_do_curl_https($https_url,$selectedPostJson,$headers) {
...
function welocally_getplaces() {
	...
	//set POST variables http://108.200.49.246/
	$url = wl_server_base().'/rcadmin/publisher/place/queryplaces.json';
...
function welocally_add_place() {
	...
	//set POST variables 
	$url = wl_server_base().'/rcadmin/publisher/place/addplace.json';
