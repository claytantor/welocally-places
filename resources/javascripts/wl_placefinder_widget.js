...
if (!WELOCALLY.PlaceFinderWidget) {
    WELOCALLY.PlaceFinderWidget = function(cfg) {
    	...
    	if (!WELOCALLY.PlaceFinderWidget._initialized) {
    			...
    			if(!cfg.loc){
    				WELOCALLY.PlaceFinderWidget._cfg.loc='38.548165_-96.064453';
    			}
    			
    			if(!cfg.zoom){
    				WELOCALLY.PlaceFinderWidget._cfg.zoom=4;
    			}
    			
    			if(!cfg.radius){
    				WELOCALLY.PlaceFinderWidget._cfg.radius=20;
    			}
        ...
        jQuery(WELOCALLY.PlaceFinderWidget._selectedSection).attr('class','wl_selected');
    ...
    WELOCALLY.PlaceFinderWidget.refreshMap = function() {
    
    	console.log('refresh');
    			
    	google.maps.event.trigger(WELOCALLY.PlaceFinderWidget._map, 'resize');
    	
    	var listener = google.maps.event.addListener(WELOCALLY.PlaceFinderWidget._map, "idle", function() { 						
			WELOCALLY.PlaceFinderWidget._map.setCenter(WELOCALLY.PlaceFinderWidget._searchLocation);								
			...
			jQuery(WELOCALLY.PlaceFinderWidget._map).show();
	...
	WELOCALLY.PlaceFinderWidget.setStatus = function(statusArea, message, type, showloading){
		...
		if(showloading){
			jQuery(statusArea).append('<div><img class="wl_ajax_loading" src="'+
					WELOCALLY.PlaceFinderWidget._cfg.imagePath+'/ajax-loader.gif"/></div>');
		}
	...
	WELOCALLY.PlaceFinderWidget.locationFieldInputHandler = function(event) {
		...
		WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, 'Geocoding','wl_update',true);
		...
		WELOCALLY.PlaceFinderWidget._geocoder.geocode( { 'address': addressValue}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK &&  WELOCALLY.PlaceFinderWidget.validGeocodeForSearch(results[0])) {
				...
				WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, '','wl_message',true);
				...
				WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, 'Could not geocode:'+status,'wl_warning',false);
			} 
		});
	...
	WELOCALLY.PlaceFinderWidget.searchHandler = function(event) {
		...
		if(WELOCALLY.PlaceFinderWidget._selectedPlace.geometry.coordinates[1] && WELOCALLY.PlaceFinderWidget._selectedPlace.geometry.coordinates[0]){
			...
			WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, 'Finding places','wl_update',true);
			...
			jQuery.ajax({
					  ...
					  success: function(data) {
						...
						if(data != null && data.length>0){						
							WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, '','wl_message',false);
						} else {
							bounds = WELOCALLY.PlaceFinderWidget._map.getBounds();
							WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, 'No results were found matching your search.','wl_warning',false);
						}
			...
			WELOCALLY.PlaceFinderWidget.setStatus(WELOCALLY.PlaceFinderWidget._ajaxStatus, 'Please choose a location for search.','wl_warning',false);
		}
