...
if (!class_exists('WelocallyPlaces')) {
	...
	class WelocallyPlaces {
		...
		function __construct() {
			...
			add_action('sp_places_post_errors', array (
				'WLPLACES_Post_Exception',
				'displayMessage'
			));
			add_action('sp_places_options_top', array (
				'WLPLACES_Options_Exception',
				'displayMessage'
			));

			add_shortcode('welocally', array (
				$this,
				'handleWelocallyPlacesShortcode'
			));
		...
		public function loadDomainStylesScripts() {
			...
			/*
			 * had problems when we did not include a version of jquery ui but now 3.5 
			 * installs are breaking
			 */
			...
			//welocally
			wp_enqueue_script('wl_base_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_base.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
			wp_enqueue_script('wl_place_widget_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_place_widget.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
			wp_enqueue_script('wl_infobox_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_infobox.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
			wp_enqueue_script('wl_places_multi_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_places_multi_widget.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
		...
		public function loadAdminDomainStylesScripts() {
			...
			//welocally
			wp_enqueue_script('wl_addplace_widget_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_addplace_wp.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
			wp_enqueue_script('wl_placesmgr_script', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_placemgr.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
			wp_enqueue_script('wl_pager', WP_PLUGIN_URL . '/welocally-places/resources/javascripts/wl_pager_wp.js', array (
				'jquery'
			), WelocallyPlaces :: VERSION);
		...
		public function handleWelocallyPlacesShortcode($attrs, $content = null) {
			...
			if (!empty ($id)) {
				...
				if (isset ($categories) || isset ($category)) {
					...
					if (!isset ($categories) && !isset ($category) && isset ($location)) {
		...
		public function databaseRequirementsMissing() {
			...
			$reqs = array ();
			if (!$this->tableExists($wpdb->prefix . 'wl_places'))
				array_push($reqs, 'table missing ' .
				$wpdb->prefix . 'wl_places');
			if (!$this->tableExists($wpdb->prefix . 'wl_places_posts'))
				array_push($reqs, 'table missing ' .
				$wpdb->prefix . 'wl_places_posts');
			if (!$this->columnExists($wpdb->prefix . 'wl_places', 'likes'))
				array_push($reqs, 'column missing ' .
				$wpdb->prefix . 'wl_places.likes');
			if (!$this->columnExists($wpdb->prefix . 'wl_places', 'lat'))
				array_push($reqs, 'column missing ' .
		...
		public function addPlaceBox() {
			...
			foreach (array (
				...
				add_meta_box('wl-place-add-meta-2', __('Welocally Add Place', 'Place_textdomain'), array (
					$this,
					'addPlaceMetaBox'
				), $type, 'normal', 'high');
			}
		...
		public function getPlaceMapMarkup($tag) {
			...
			$place = $wpdb->get_row($wpdb->prepare("SELECT {$wpdb->prefix}wl_places.* " .
		...
		public function getPlaces($cat = null, $maxElements = 25, $filter = true, $post_type = 'post', $distance, $location, $titlelink = true) {
			...
			elseif (is_object($cat)) $cat = $cat->cat_ID;
			...
			if (isset ($distance) && isset ($location) && (!empty ($cat) && $cat != $this->placeCategory())) {
				...
				if (isset ($distance) && isset ($location) && ($cat == $this->placeCategory() || empty ($cat))) {
		 ...
		 */
		public function getCategoryMapMarkup($cat = null, $template = null, $showIfEmpty = null, $maxElements = 25, $post_type = 'post', $distance, $location, $titlelink = true) {
		...
		*/
		...
		public function addPlaceMeta($postId, $action) {
			...
			if (!empty ($_POST['deletePlaceInfo'])) {
				...
				if (!empty ($_POST['PlaceSelected'])) {
					...
					if (isset ($_POST['PlaceCategoriesSelected'])) {
						...
						$cats = wp_get_object_terms($postId, 'category', array (
							'fields' => 'ids'
						));
						$new_cats1 = array_merge(array (
							$category_id
						), $cats);
			...
			/*else if($_POST['isWLPlace'] == 'true') {
				...
				if ($_POST['isWLPlace'] == 'true' && empty ($_POST['PlaceSelected'])) {
					error_log("save case 2", 0);
					update_post_meta($postId, '_isWLPlace', 'false');
				} else
					if ($_POST['isWLPlace'] == 'false' || !isset ($_POST['isWLPlace'])) {
		...
		public function getOptions() {
			...
			//widget
			if (!array_key_exists('show_letters', $options)) {
				$options['show_letters'] = $default_show_letters;
				$changed = true;
			}
			if (!array_key_exists('show_selection', $options)) {
				$options['show_selection'] = $default_show_selection;
				$changed = true;
			}
			if (!array_key_exists('infobox_title_link', $options)) {
				$options['infobox_title_link'] = $default_infobox_title_link;
				$changed = true;
			}
			if (!array_key_exists('widget_post_type', $options)) {
				$options['widget_post_type'] = $default_widget_post_type;
				$changed = true;
			}

			//tag
			if (!array_key_exists('show_letters_tag', $options)) {
				$options['show_letters_tag'] = $default_show_letters_tag;
				$changed = true;
			}
			if (!array_key_exists('show_selection_tag', $options)) {
				$options['show_selection_tag'] = $default_show_selection_tag;
				$changed = true;
			}
			if (!array_key_exists('infobox_title_link_tag', $options)) {
				$options['infobox_title_link_tag'] = $default_infobox_title_link_tag;
				$changed = true;
			}

			//selectable style overrides
			if (!array_key_exists('widget_selection_style', $options)) {
				$options['widget_selection_style'] = $default_widget_selection_style;
				$changed = true;
			}
			if (!array_key_exists('tag_selection_style', $options)) {
				$options['tag_selection_style'] = $default_tag_selection_style;
				$changed = true;
			}

			//theme	
			if (!array_key_exists('places_theme', $options)) {
				$options['places_theme'] = $default_places_theme;
				$changed = true;
			}
		...
		public function geoSearchWithCategory($units = 'km', $max = 10, $cat, $post_type = 'post', $dist = 10.0, $location = '37.8068958_-122.2693737') {
			...
			"ORDER BY rawgeosearchdistance ASC LIMIT 0,%d", $r, $loc['lat'], $loc['lat'], $loc['lng'], $lat1, $lat2, $lng1, $lng2, $dist, $max);
